...
public class Asteroid extends Participant implements BulletDestroyer, ShipDestroyer, AlienBulletDestroyer, AlienDestroyer
{
    ...
    public void collidedWith (Participant p)
    {
        if (p instanceof AsteroidDestroyer)
        {
            ...
            if(this.size == 2)
            {
                Clip bigBoom = test.createClip("/sounds/bangLarge.wav");
                if ( bigBoom != null)
                {
                    if (bigBoom.isRunning())
                    {
                        bigBoom.stop();
                    }
                    bigBoom.setFramePosition(0);
                    bigBoom.start();
                }
            }
            else if (this.size == 1)
            {
                Clip mediumBoom = test.createClip("/sounds/bangMedium.wav");
                if ( mediumBoom != null)
                {
                    if (mediumBoom.isRunning())
                    {
                        mediumBoom.stop();
                    }
                    mediumBoom.setFramePosition(0);
                    mediumBoom.start();
                }
            }
            else if (this.size == 0)
            {
                Clip smallBoom = test.createClip("/sounds/bangSmall.wav");
                if ( smallBoom != null)
                {
                    if (smallBoom.isRunning())
                    {
                        smallBoom.stop();
                    }
                    smallBoom.setFramePosition(0);
                    smallBoom.start();
                }
            }
